...
- **`/keepwarm` skill** (`share/skills/keepwarm/`): bounded cache keep-warm loop for planned absences. `/keepwarm [interval] [max]` (defaults 55 min × 6; interval clamped [1,59], cap [1,24]) schedules minimal self-wake turns that re-read the cached conversation prefix before the TTL expires, avoiding the full cache rewrite on return. Arming always announces interval, cap, per-ping cost, auto-stop time, and the cancel phrase; real user activity resets the counter. Pre-arm probe (`keepwarm-calc.sh`, jq-only) warns on 5-minute-tier sessions with too-long intervals and on tiny sessions not worth pinging. Stop-hook architecture was evaluated and rejected — Stop hooks block the TUI for their whole runtime (see `docs/PHASE-8-KEEPWARM-PLAN.md` S0). `install.sh` deploys the skill (skip with `--no-keepwarm`); `uninstall.sh` removes it. Tests: `tests/test_keepwarm_skill.bats` (15) + 2 e2e probe assertions.
