...
- once the checkpoint is confirmed on disk, types the one thing the model can't do for itself — `/clear` — then a short resume nudge. Your `SessionStart` auto-read hook reloads `RESUME.md`, so work continues from where it left off. The resume nudge waits for **proof** that the clear ran (the transcript rotating on disk), never for a timer: Claude Code *queues* input typed during a running turn, and a resume typed on a timer was consumed by the still-live old session before the queued `/clear` fired and wiped it, leaving the new session with no instruction at all. One invariant holds the whole path together — **a session is never left cleared but uninstructed**: once `/clear` is typed, either the resume follows or the session is closed. A stand-down marker, a `pause`, or a `/checkpoint --final` landing mid-clear each resolve the cycle explicitly instead of dropping it, and if the rotation never comes within `--clear-timeout` the watcher gives up **without** resuming rather than typing into a session that never cleared;
...
- one gap the occupancy ladder can't cover: a nudged session that goes **completely quiet without printing the sentinel** (observed in the wild: the model checkpoints, then hands the "safe to /clear" decision back to a human who isn't there). Occupancy never moves on an idle session, so a **silent-stall watchdog** fires after `CCAGE_AUTOCK_STALL_IDLE` seconds (default 600) of zero transcript growth since the nudge: a loud log line plus one firm, honest re-nudge ("print the sentinel; the session cannot clear without it") — then rate-limited log entries only, never a nag loop. Idleness is the trigger, so it cannot interrupt a session that's actually working; `CCAGE_AUTOCK_STALL_IDLE=0` disables it.

- the ladder above manages **context**, and a session can also stop for the opposite reason: it goes **idle with work still outstanding**. Measured: a session stopped at 23:33 with a detached job still running, the job finished at 23:48, and the session sat at the prompt for eleven hours. The Stop hook is the only intervention point and it fires once, at the instant of stopping; nothing ever asks again. So the watcher runs a **bounded supervisor** alongside the occupancy ladder. When the transcript has been quiet for `CCAGE_AUTOCK_SUPERVISOR_IDLE` seconds (default 300) it asks **once** — stating the observation and offering four honest answers: carry on, run `/checkpoint --final` if there is genuinely nothing left, arm a `ccage-watch` spec if the wait is on a **machine**, or declare `ccage-watch hold --question '…'` if the wait is on **you**. That last one exists because the other three cover machine waits only: `--cond` needs a command that exits 0, and there is none for "a human must do this", so a blocked session used to have to pick a falsehood — and the one it picked was *carry on*, manufacturing speculative work. A hold is silent until you type or its TTL lapses, and it has to name the question, which is written straight into `RESUME.md` so it survives the very `/clear` the ladder might perform. Still quiet `CCAGE_AUTOCK_SUPERVISOR_ESCALATE` seconds later, it converts the session once (checkpoint → `/clear` → resume) rather than nagging. After that the **circuit opens** and it stays silent until the session does real work again — and "real work" means a tool call, not a reply, so a session that answers "still waiting" and stops does not earn a fresh round. It never infers that the work is finished; it asks, and accepts a *command* as the answer, because a marker file is a fact and a phrase is not. If a background job is still growing and a watcher is armed for it, the supervisor stays quiet — the wait has been declared. If a job has **landed unread**, it speaks up naming the file, because reading the result is the whole point. Set `CCAGE_AUTOCK_SUPERVISOR=0` to turn it off.
...
| Silent-stall watchdog (s idle; 0 = off) | — | `CCAGE_AUTOCK_STALL_IDLE` | 600 |
| Idle-session supervisor on/off | — | `CCAGE_AUTOCK_SUPERVISOR=0` | on |
| Supervisor: quiet before it asks (s) | — | `CCAGE_AUTOCK_SUPERVISOR_IDLE` | 300 |
| Supervisor: quiet after asking, before it restarts (s) | — | `CCAGE_AUTOCK_SUPERVISOR_ESCALATE` | 300 |
| Close the session when the work is marked done | `--exit-on-done` / `--no-exit-on-done` | `CCAGE_AUTOCK_EXIT_ON_DONE=0` | on |
| Done-close: transcript quiet before `/exit` (s) | — | `CCAGE_AUTOCK_DONE_GRACE` | 10 |
| How long to wait for `/clear` to actually run (s) | `--clear-timeout N` | `CCAGE_AUTOCK_CLEAR_TIMEOUT` | 300 |
...
**Standing down when the work is done.** The watcher runs its checkpoint→clear→resume loop indefinitely — it has no way to know the *task* is finished, only that the *context* is full. To let an autonomous run terminate itself, the model runs `/checkpoint --final` when its objective is complete; that writes a `.ccage-session-done` marker and `ccage-auto` closes the Claude session for you — it waits for the turn that wrote the marker to finish (`--final --tidy` keeps working after it), types `/exit`, and falls back to `SIGHUP` then `SIGKILL` if the session won't go. You land back at your own shell prompt; the terminal window and any SSH session stay up. Opt out with `--no-exit-on-done` / `CCAGE_AUTOCK_EXIT_ON_DONE=0`, or tune the quiet period with `CCAGE_AUTOCK_DONE_GRACE` (default 10s). So for a hands-off run, end your kickoff prompt with an instruction like *"when the objective is fully complete, run `/checkpoint --final`."* Without it the watcher keeps the loop alive until you stop it (Ctrl-C). The marker is cleared automatically at the next session start, so it never carries over.

The watcher can also write that marker **itself**, so a hands-off run is not hostage to the model remembering the instruction — but only on facts, never on a claim. It needs a checkpoint actually confirmed on disk, nothing running, and the supervisor's own escalation cycle to be the one that produced it. A session that simply went quiet and never answered does **not** get a marker: that would turn its meaning from *"the model confirmed nothing is left"* into *"nobody replied"*, and the marker silences the watcher entirely. The asymmetry is deliberate throughout — anything the model writes can wake the supervisor up, and only a fact on disk can send it to sleep.

**Writing a kickoff prompt that stays on the rails.** Session forensics comparing a near-perfect unattended run against routine stallers found the difference was the *prompt contract*, not the model: the good run's `RESUME.md` pinned **one in-scope item**, a **binding definition of done** ("this run is DONE only when …", concrete and checkable), and **pre-declared deferrals** (every user-gated item named out-of-scope up front). A kickoff prompt (`CCAGE_AUTOCK_INIT_PROMPT` or typed) that carries — or points at — those three things gets an auditable run: any end-of-run "waiting on you" claim can be checked against a declaration made *before* the work started.
...
# Hold: the wait is on a PERSON, so there is no condition to poll.
ccage-watch hold --question 'run the attended live-fire — I cannot authenticate'

ccage-watch list       # what is armed or held
...
**`hold` is the same idea for a wait `--cond` cannot express.** Some waits are on a human — a decision, a credential, a command only they can run — and there is no shell command that exits 0 when a person gets round to it. Before `hold`, a session in that position had to misrepresent itself to `ccage-auto`'s idle supervisor, and what it actually chose was *carry on*: it invented work rather than admit it was stuck. A hold spawns no process; it records the question in `RESUME.md` at once (so it survives a `/clear`) and keeps the supervisor quiet until you type or its TTL lapses — 72 h by default, because a person answering is slower than a job finishing. It is not a mute button: the session cannot end the silence itself, `--question` is mandatory, and an unanswered hold is reported at the next session start as still open rather than resolved.
